...
- 選定の目安（参考）：受信は 1–10us 程度の量子化を想定。エアコン等の長大波形を含む場合は 5us 程度、短いリモコン中心なら 1us 近辺を想定。プロトコルで分解能が規定される場合（例：NEC送信）はその値を用いる。`T_us` を小さくすれば誤差減・データ長増、大きくすればデータ短縮・誤差増となる。データ長最小化のために無理に最適化することは非推奨。
