...
- AC関連の詳細仕様（状態モデル/Intent/Capabilities/バリデーション）は `SPEC_AC.ja.md` に分離。ここではACのAPI概略のみを扱う。
...
```
- プロトコル別の送信ヘルパ（例：`tx.sendNEC`）は「対応プロトコルとヘルパー」を参照。`gapUs` はユーザー設定があればそれを、なければヘルパが持つ推奨値（なければ既定40ms）を適用する。
...
- 対応状況（○=実装＋確認済み、▲=実装済み/未テスト、△=枠のみ/予定、RAWはITPS直扱い）

| プロトコル                 | フレーム構造体                    | デコードヘルパ                   | 送信ヘルパ                                  | 状態 |
|---------------------------|----------------------------------|---------------------------------|--------------------------------------------|------|
| RAW (ITPS)                | なし（ITPSBuffer直接）            | -                               | `tx.send(ITPSBuffer)`                      | ○    |
...
| AEHA (家電協)             | `esp32ir::payload::AEHA`         | `esp32ir::decodeAEHA`           | `tx.sendAEHA(struct/args)`                 | ▲    |
...
| Apple (NEC拡張系)         | `esp32ir::payload::Apple`        | `esp32ir::decodeApple`          | `tx.sendApple(struct/args)`                | ▲    |
...
- 構造体とヘルパー詳細
  ...
  - NEC  
    ...
    - `bool esp32ir::Transmitter::sendNEC(const esp32ir::payload::NEC& p);` / `bool esp32ir::Transmitter::sendNEC(uint16_t address, uint8_t command, bool repeat=false);`
...
- 参考推奨値（`setGapUs` 未設定時にヘルパーが使う目安値）  
  ...
  - RC5 / RC6: 30ms  
  - AC 系 (Daikin / Panasonic / Mitsubishi / Toshiba / Fujitsu): 50ms  
  - その他未分類: 40ms  
...
- `status`：任意。`DECODED` | `RAW_ONLY` | `OVERFLOW`（`messageBytes` があればデコード済みとみなせる）
...
```json
{
  ...
  "capture": {
    ...
    "itps": [
      { "T_us": 10, "flags": 0, "seq": [127,127,127,127,127,127,127,11,-127,-127,-127,-69,56,-56,56,-56,56,-127,-42,56,-56,56,-56,56,-56,56,-56,56,-56,56,-56,56,-56,56,-56,56,-56,56,-56,56,-56,56] }
    ]
  },
  "expected": {
    "protocol": "NEC",
