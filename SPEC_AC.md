...
- Layers: Intent → Normalized DeviceState → Protocol Frame (bytes/bits) → IR Waveform (ITPS) → TX. ITPS handling follows `SPEC.md` (polarity absorbed by HAL). Capture JSON format is now documented in `SPEC.md`.
